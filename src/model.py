...
class PMModelTrainer:
    ...
    def load_data(self):
        ...
        df = df.dropna()
        self.X = df.drop(columns=['PM2.5', 'PM10'])
    ...
    def train_model(self, target):
        ...
        logging.info(f'{target} model MSE: {mse:.4f}')
        logging.info(f'{target} model RMSE: {rmse:.4f}')
        logging.info(f'{target} model MAE: {mae:.4f}')
        logging.info(f'{target} model R²: {r2:.4f}')

        joblib.dump(model, os.path.join(self.model_dir, f'rf_model_{target.lower()}.pkl'))

        self.plot_predicted_vs_actual(model, X_test, y_test, target)
        self.plot_residuals(y_test, y_pred, target)
    ...
    def execute(self):
        self.load_data()
        self.train_model('PM2.5')
        self.train_model('PM10')
